...
차량 할당부터 실시간 운행 관제, 반납 후 운행일지 자동 기록까지 체계적으로 관리할 수 있으며, 사원에게 차량을 쉽게 배정하고 운행 중인 차량의 위치와 상태를 실시간으로 모니터링하여 차량 관리의 편의성을 제공합니다.

## ✨ 핵심 기능

### 👥 사용자 관리
- **회원가입 및 로그인**: 관리자와 사원 권한 분리
- **역할 기반 접근**: 회원 유형에 따른 다른 기능/화면 제공

### 🚙 차량 관리
- **차량 등록 및 관리**: 회사 내부 차량 정보 등록 및 상태 변경
- **사원 차량 할당**: 사원 정보 확인 및 차량 할당 기능
- **할당 정보 조회**: 할당 정보와 연관된 운행 기록 조회

### 📡 실시간 관제
- **실시간 위치 추적**: SSE(Server Sent Event)를 통한 준실시간 차량 위치 추적
- **이동 경로 표현**: 발생 좌표 정보를 기반으로 한 실시간 이동 경로 시각화
- **애뮬레이터**: RabbitMQ를 활용한 주기정보 및 이벤트 정보 전송 (Producer → Broker → Consumer)
...
- **메시지 큐**: RabbitMQ를 활용한 실시간 데이터 처리
- **실시간 통신**: Redis + SSE(Server Sent Event)를 통한 클라이언트-서버 간 실시간 데이터 전송
- **애뮬레이터**: 차량 위치 및 상태 정보 시뮬레이션
